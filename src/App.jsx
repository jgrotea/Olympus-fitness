...
const RETENTION_RITUALS = [
  { key:"magnesio", label:"Magnesio", desc:"400mg antes de dormir", icon:"𖤓" },
  { key:"platano", label:"Potasio · Plátano", desc:"1–2 al día, reduce sodio extracelular", icon:"𖤐" },
...
function ls_get(key) { try { const v = localStorage.getItem(key); return v ? JSON.parse(v) : null; } catch { return null; } }
function ls_set(key, val) { try { localStorage.setItem(key, JSON.stringify(val)); } catch {} }
function ls_del(key) { try { localStorage.removeItem(key); } catch {} }
...
export default function Olympus() {
  ...
  const retDone = RETENTION_RITUALS.filter(t=>retChecks[t.key]).length;
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          messages:[{ role:"user", content:`Analiza estas comidas para persona de ${profile.weight}kg, ${profile.height}cm, en fase "${profile.phase}". Responde SOLO JSON sin backticks:\n{"items":[{"name":"","calories":0,"protein":0,"sodium_mg":0}],"total_calories":0,"total_protein":0,"total_sodium":0,"retention_warning":"","tip":""}\n\nComidas: ${foodText}` }]
        })
      });
      ...
      setFoodText("");
    } catch { notify("Error analizando comida"); }
  ...
  async function consultOracle() {
    ...
    try {
      const l7 = last7();
      const ctx = { profile, avg_burned:Math.round(l7.reduce((s,d)=>s+d.burned,0)/7), avg_eaten:Math.round(l7.reduce((s,d)=>s+d.eaten,0)/7), avg_water:((l7.reduce((s,d)=>s+d.water,0)/7)/1000).toFixed(1)+"L", retention_today:`${retDone}/${RETENTION_RITUALS.length}`, photos_count:photos.length };
      const content = photos.length>0&&photos[0].data ? [
        ...
        { type:"text", text:`Coach experto en composición corporal. Analiza la foto y datos:\n${JSON.stringify(ctx)}\n\nFase: ${profile.phase}\nMeta: ${profile.goal}\n\nResponde con secciones:\n## ANÁLISIS VISUAL\n## DIAGNÓSTICO\n## RECOMENDACIONES INMEDIATAS\n## ALERTAS\n## SIGUIENTE OBJETIVO MEDIBLE (14 días)\n\nSé directo, profesional y específico.` }
      ] : `Coach experto en composición corporal. Analiza estos datos:\n${JSON.stringify(ctx)}\n\nFase: ${profile.phase}, Meta: ${profile.goal}\n\n## DIAGNÓSTICO\n## RECOMENDACIONES INMEDIATAS\n## ALERTAS\n## SIGUIENTE OBJETIVO MEDIBLE (14 días)\n\nSé directo. Recomienda subir foto para análisis visual.`;
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1200, messages:[{ role:"user", content }] })
      });
      const data = await res.json();
      setOracleAdvice(data.content[0].text);
    } catch { notify("Error del oráculo, intenta de nuevo"); }
  ...
  function last7() {
    return Array.from({length:7},(_,i)=>{
      ...
      const log = logs[key]||{};
      const di = d.getDay();
      return { day:DAYS[di===0?6:di-1], burned:(log.activities||[]).reduce((s,a)=>s+a.calories,0), eaten:(log.foods||[]).reduce((s,f)=>s+f.calories,0), weight:log.weight||null, water:log.water||0 };
    });
  }
  ...
  const tabBtn = (id,label) => (
    <button key={id} onClick={()=>setTab(id)} style={{
      ...
      color:tab===id?GOLD:"#888", fontFamily:fontH, fontSize:10,
      ...
      <div style={{ background:`linear-gradient(180deg,#1c1814,${BG_DARK})`, borderBottom:`1px solid ${STONE}`, padding:"24px 16px 0", position:"relative" }}>
        ...
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:32, padding:"16px 0 12px" }}>
            ...
            <div style={{ width:1, background:STONE, height:30 }}/>
            <Stat label="RITUAL" value={`${retDone}`} unit={`/${RETENTION_RITUALS.length}`}/>
          </div>
          <div style={{ display:"flex", justifyContent:"center", gap:2, overflowX:"auto", borderTop:`1px solid ${STONE}` }}>
            {[["today","Hoy"],["retention","Rituales"],["nutrition","Ambrosía"],["progress","Progreso"],["oracle","Oráculo"],["stats","Crónicas"],["settings","Templo"]].map(([id,l])=>tabBtn(id,l))}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding:"20px 16px", maxWidth:600, margin:"0 auto" }}>
        ...
        {tab==="today" && <>
          ...
          <div style={panel}>
            ...
            <div style={{ fontFamily:"Inter,sans-serif", fontSize:10, color:"#666", marginTop:6 }}>{(water/1000).toFixed(2)}L de 3L · cada barra ≈ 300ml</div>
          </div>
          <div style={panel}>
            ...
            {showActForm && (
              <div style={{ background:"#0e0c0a", border:`1px solid ${STONE}`, padding:14, marginTop:10, marginBottom:10 }}>
                ...
                {actForm.type&&actForm.duration&&<div style={{ color:GOLD, fontFamily:fontB, fontStyle:"italic", fontSize:14, marginBottom:10 }}>≈ {calcCal(actForm.type,parseFloat(actForm.duration),profile.weight)} kcal quemadas</div>}
        ...
        {tab==="retention" && <>
          <div style={panel}>
            ...
            <div style={{ marginTop:14 }}>
              <span style={{ fontFamily:fontH, fontSize:11, color:retDone===RETENTION_RITUALS.length?GOLD:"#888", letterSpacing:3 }}>{retDone}/{RETENTION_RITUALS.length} CUMPLIDO {retDone===RETENTION_RITUALS.length?"· VIRTUOSO":""}</span>
            </div>
            <ProgressBar value={retDone} max={RETENTION_RITUALS.length} color={GOLD}/>
          </div>
          {RETENTION_RITUALS.map(tip=>(
          ...
          <div style={panel}>
            <SectionTitle icon="⚕" title="Tu Stack Actual"/>
            {[["✓ Magnesio","400mg antes de dormir · óptimo"],["✓ Plátano (Potasio)","1–2 al día · perfecto"],["+ Sumar: Arándanos","Antiinflamatorio natural, reduce edema"],["+ Sumar: Té Diente de León","Diurético suave y efectivo"],["+ Sumar: Pepino con limón","Diurético natural, bajo en kcal"]].map(([n,d],i)=>(
              <div key={i} style={{ padding:"10px 0", borderBottom:i<4?`1px solid ${STONE}`:"none" }}>
                <div style={{ fontFamily:fontH, fontSize:11, color:n.startsWith("✓")?GOLD:MARBLE, letterSpacing:2, textTransform:"uppercase" }}>{n}</div>
                <div style={{ fontFamily:fontB, fontSize:13, fontStyle:"italic", color:"#777", marginTop:2 }}>{d}</div>
              </div>
            ))}
          </div>
        </>}
        ...
        {tab==="nutrition" && <>
          ...
          {foodResult && (
            <div style={panel}>
              ...
              {foodResult.tip && <div style={{ marginTop:10, fontFamily:fontB, fontStyle:"italic", color:GOLD, fontSize:14 }}>◆ {foodResult.tip}</div>}
            </div>
          )}
            ...
            <div style={panel}>
              ...
              {(todayLog.foods||[]).map((f,i)=>(
                <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:`1px solid ${STONE}` }}>
                  ...
                  <span style={{ fontFamily:fontH, color:GOLD, fontSize:13, flexShrink:0 }}>{f.calories} kcal</span>
                </div>
              ))}
        ...
        {tab==="progress" && <>
          <div style={panel}>
            ...
            <p style={{ fontFamily:fontB, fontStyle:"italic", color:"#666", fontSize:13, marginTop:10, lineHeight:1.5 }}>
              Consejo: misma hora, mismo lugar, misma iluminación. Idealmente en ayunas para comparar semana a semana.
            </p>
          </div>
          {photos.length===0
